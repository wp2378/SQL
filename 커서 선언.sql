// 부서아이디를 전달받아서 해당부서의 총 연봉을 반환하는 함수
...
IS
    ...
    -- 커서 선언
    -- 커서는 SELECT문의 실행결과가 반환되는 결과값을 저장하는 메모리 공간을 참조하는 참조변수다.
...
BEGIN
    -- 커서(EMP_LIST)에 담겨있는 데이터행은 FRO문을 사용해서 한 행이 꺼낸다.
    -- EMP는 EMP_LIST에 담겨잇는 데이터행이 하나씩 저장되는 변수다.
