...
스키마(SCHEMA)
    ...
    * 다른 사용자가 소유하고 있는 데이터베이스객체를 액세스할 때는 반드시 스키마를 명시해야 한다.
*/
...
시노님을 사용하는 이유
    ...
    시소님을 사용하면 사용자가 참조하고 있는 객체에 대한 소유자, 이름을 감출 수 있다.
...
-- HR계정의 EMPLOYEES 테이블을 HTA에 사용하는 경우
/*
...
HTA 계정으로 접속해서 EMPLOYEES 조회하기 (시노님 생성전 방법)
    SELECT *
    FROM HR.EMPLOYEES
    
    
HTA 계정으로 접속해서 시노님을 생성하기 (시노님 생성전 방법)
    CREATE SYNONYM EMP 
    FOR HR.EMPLOYEES;
