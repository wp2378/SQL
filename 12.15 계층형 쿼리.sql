/*
계층형 쿼리
    테이블에 계층형 테이터가 존재하는 경우 그 데이터를 조회하기 위해서 사용하는 쿼리다.
    * 계층형 데이터랑 동일한 테이블에 계층적으로 상위와 하위 개념이 포함된 데이터다.
    * 예시) 직원테이블에 사원과 관리자 
           조직도테이블에서 상위조직과 하위조직
           메뉴테이블에서 상위메뉴와 하위메뉴
           카테고리테이블에서 상위 카테고리와 하위카테고리
    ...
    형식
        ...
        * CONNECT BY : 부모행과 자식행 간의 관계과 있는 컬럼을 지정한다.
...
SELECT LEVEL,LPAD(' ',4*(LEVEL-1), ' ') || EMPLOYEE_ID,
